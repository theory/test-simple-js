    ...
    Test.Harness.Browser.prototype.runTest = function (file, buffer) {
        ...
        if ( fileType == 'html' ) {
            ...
            if (/MSIE|Safari|Opera|Konqueror/.test(navigator.userAgent)) {
                ...
                var needXhrHack = true
            }
            else {
                // See whether we can use the DOM (document.write() won't
                // work) to create a new document with script elements for all
                // of the JavaScript scripts we want to run. In some browsers
                // not checked for above, document.open() does not reset the
                // global object. Patch from Father Chrysostomos.
                try {
                    var varname = "I hope this does not exist."
                    while(varname in buffer)
                        varname += 'z';  // utter paranoia
                    buffer[varname] = 'dwit'
                    var doc = buffer.document;
                    doc.open("text/html");
                    doc.close();
                    if(buffer[varname] == 'dwit') needXhrHack = true
                } catch(e) { needXhrHack = true }
            }

            // Do the XHR hack if necessary
            if(needXhrHack) {
